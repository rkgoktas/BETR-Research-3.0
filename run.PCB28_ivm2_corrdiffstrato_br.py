...
runID = runID = ['PCB28_ivm2_corrdiffstrato_br'] # output names 
years = [range(1930, 1932)]*len(runID)    # range of modeling run (years  A.D.)

emisdir = ['PCB28_HIGH_1500_mol_per_hour']*len(runID)  # emission inventory ('Emissions/annual/') 
seasparfile = ['seasonal_parameters_20C3M.a2.comp.ivm.moh_add2mix_br.txt']*len(runID)#  seasonally varying parameters ('Environment/)
constparfile = ['const_parameters_20C3M.comp.ivm_corrdiffstrato_br.txt']*len(runID)  # seasonally constant parameters ('Environment/')
...
chemnr = [4]*len(runID)  # selection of chemical from chemical properties files
...
for i in range(0, len(runID)):
    ...
    ## nc-file output
    m.dyn_res = result           
    m.output_dyn(filename = 'OUT', units = ['mol','kg','mol_per_m3','kg_per_m3','Pa'], netcdf = True, cpk = True)
